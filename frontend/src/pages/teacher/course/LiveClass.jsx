...
import {
  ...
  useUpdateMeetingStatusMutation,  // We need to add this
...
const LiveClass = () => {
  ...
  const [updateMeetingStatus] = useUpdateMeetingStatusMutation();  // Add this for status update

  const { data: meetingData, refetch } = useGetMeetingByCourseQuery(courseId, {
  ...
  const handleScheduleMeeting = async () => {
    try {
      ...
      toast.success(`Meeting scheduled for ${new Date(scheduledDate).toLocaleString()}`);
      setScheduledDate("");
  ...
  const handleDeleteMeeting = async (meetingId) => {
    try {
      ...
      setLocalMeetings((prev) => prev.filter((meeting) => meeting._id !== meetingId));
      refetch();
    } catch (err) {
      console.error("Error:", err);
  ...
  const handleStartMeeting = async (meetingId) => {
    try {
      await updateMeetingStatus(meetingId).unwrap();  // Trigger the status change
          ...
          {localMeetings.map((meeting, index) => (
            <Card key={meeting._id} className="shadow-md hover:shadow-lg transition">
              <CardContent className="py-4 space-y-2">
                <div className="flex justify-between items-center">
                  <div>
                    <h4 className="text-lg font-medium">Meeting #{index + 1}</h4>
                  ...
                  <div className="flex gap-2">
                    {meeting.status === "scheduled" ? (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleStartMeeting(meeting._id)}  // Start the meeting
                      >
                        Enter Room
                      </Button>
                    ) : (
